...
def create_app():
    ...
    admin.add_view(
        AdminModelView(ContactMessage, db.session, name="ContactMessageAdmin")
    )
    ...
    def register():
        ...
        if form.validate_on_submit():
            ...
            user = User(
                ...
                role="admin"
                if form.email.data.endswith("@admin.com")
                else "regular",  # Assign role based on email domain
            )
    ...
    def profile():
        ...
        return render_template(
            "profile.html", form=form, user=current_user, edit_mode=edit_mode
        )
    ...
    def update_book_details(book_id):
        ...
        if form.validate_on_submit():
            ...
            book.categories = [
                Category.query.get(category_id) for category_id in form.categories.data
            ]
            # Commit the changes to the database
            db.session.commit()
            # Flash a success message and redirect to the book detail page
        ...
        if request.method == "GET":
            ...
            form.categories.data = [
                category.category_id for category in book.categories
            ]
